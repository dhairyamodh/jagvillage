...
if (isset($_POST['submit'])) {
	...
	if ($_POST['category'] != "") {
		...
		$empty = array();
		$data = array();
		$time_one = date("H:i", strtotime($_POST['time1']));
		$time_two = date("H:i", strtotime($_POST['time2']));
		foreach ($period as $key => $value) {
			$getcurday = date('D', strtotime($value->format('Y-m-d')));
			foreach ($getdays as $getdaykey => $getdaysval) {
				if ($getdaysval == $getcurday) {
					$cate_date = "insert into category_date (service_token,category,date,reminder,time_one,time_two) values('$token','" . mysqli_real_escape_string($con, $_POST["category"]) . "','" . $value->format('Y-m-d') . "','" . mysqli_real_escape_string($con, $_POST["remind"]) . "','" . $time_one . "','" . $time_two . "')";
...
<body>
	...
	<div class="modal fade addRec" id="event-modal" tabindex="-1" role="dialog" id="addcalendar" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
				...
				<div class="modal-body">
					...
					<form action="#" method="post">
						<label for="phone" class="wizard-form-text-label">Select Category<span class="service-err">
							</span></label>

						<select name="category" class="wizard-required" required>
						...
						<div class="row">
							...
							<div class="col-md-7" style="padding:0; display: flex; justify-content:space-between;align-items:center">
								<div class="col-md-6">
									<input type="text" name="time_one" class="timepicker" required readonly />
								</div>
								<p style="text-align:center">to</p>
								<div class="col-md-6">
									<input type="text" name="time_two" class="timepicker" required />
								</div>
							</div>
						</div>
						<label for="phone" class="wizard-form-text-label">Repeat<span class="repeat-err">
							</span></label>
						<select name="repeat" required style="margin-bottom: 10px;">
						...
						<div class="on-week" style="display: none;margin-top: 10px;">
							<div class="round-checkbox">
								<div class="round">
									<input type="checkbox" id="sun" value="Sun" name="days" />
									<label for="sun">S</label>
								</div>
								<div class="round">
									<input type="checkbox" id="mon" value="Mon" name="days" />
									<label for="mon">M</label>
								</div>
								<div class="round">
									<input type="checkbox" id="tue" value="Tue" name="days" />
									<label for="tue">T</label>
								</div>
								<div class="round">
									<input type="checkbox" id="wed" value="Wed" name="days" />
									<label for="wed">W</label>
								</div>
								<div class="round">
									<input type="checkbox" id="thu" value="Thu" name="days" />
									<label for="thu">T</label>
								</div>
								<div class="round">
									<input type="checkbox" id="fri" value="Fri" name="days" />
									<label for="fri">F</label>
								</div>
								<div class="round">
									<input type="checkbox" id="sat" value="Sat" name="days" />
									<label for="sat">S</label>
								</div>
							</div>
						</div>
						<div class="until" style="margin-bottom: 5px; display: none;">
							<small>Occurs <span class="every"></span> unit </small><input type="text" name="end_date" class="end_date" required style="background-color: #fff;border:none;width:80px;margin:0;padding:0;cursor:pointer;color:#159397;" />
							<span style="color: #159397;" class="caret"></span>
						</div>
						<label for="phone" class="wizard-form-text-label">Remind me</label>
						<select name="remind" required>
							<option selected value="never">Never</option>
				...
				<div class="modal-footer">
					...
					<button type="button" id="add_service" class="btn btn-primary">Save Changes</button>
					</form>
				</div>
			</div>
			<!-- /.modal-content -->
	...
	if (!empty($_SESSION['email'])) {
		...
		<script>
			$('select[name="repeat"]').change(function() {
				function set_end(repeat) {
					var start_date = $('.date').datepicker("getDate")
				...
				if (val == 'day') {
					...
					$('.until').css('display', 'block')
					$('.every').text('every week')
					$('.on-week').css('display', 'block')
					$('.end_date').datepicker(
						"setDate", set_end('week'),
					);
				} else if (val == 'month') {
					$('.until').css('display', 'block')
					$('.every').text('every month')
					$('.on-week').css('display', 'none')
					$('.end_date').datepicker(
						"setDate", set_end('month'),
					);
				} else if (val == 'year') {
					$('.until').css('display', 'block')
					$('.every').text('every year')
					$('.on-week').css('display', 'none')
					$('.end_date').datepicker(
						"setDate", set_end('year'),
					);
				} else {
					$('.on-week').css('display', 'none')
					$('.until').css('display', 'none')
				}
			})
			...
			$('input[name="time_one"]').timepicker({
				timeFormat: 'h:i A',
			}).val('12:00 PM');
			$('input[name="time_two"]').timepicker({
				timeFormat: 'h:i A',
			}).val('12:30 PM');
			$(document).ready(function() {
				function addCalendarEvent(id, start, end, title, description) {
					var eventObject = {
						id: id,
						start: start,
						end: end,
				...
				$('#add_service').on('click', function() {
					...
					if ($('select[name="category"]').val() == null) {
						...
						$('.service-err').html('<span class="text-danger" style="margin-left:10px;font-size:0.85em">Please select category</span>')
						$('.repeat-err').html('')
					} else if ($('select[name="repeat"]').val() == "never") {
						$('.service-err').html('')
						$('.repeat-err').html('<span class="text-danger" style="margin-left:10px;font-size:0.85em">Please select repeat</span>')
					} else {
						var days = []
						...
						$.ajax({
							...
							data: {
								load_dates: 1,
								title: $('select[name="category"]').val(),
								start: $('input[name="start_date"]').val(),
								end: $('input[name="end_date"]').val(),
								time_one: $('input[name="time_one"]').val(),
								time_two: $('input[name="time_two"]').val(),
								days: days,
								repeat: $('select[name="repeat"]').val(),
								remind: $('select[name="remind"]').val()
							},
							success: function(data) {
								...
								for (var i = 0; i < data.length; i++) {
									...
									addCalendarEvent(i, data[i].start, data[i].end, data[i].title, data[i].description);
									$('input[name="category"]').val($('select[name="category"]').val())
									$('input[name="start"]').val($('input[name="start_date"]').val())
									$('input[name="end"]').val($('input[name="end_date"]').val())
									$('input[name="time1"]').val($('input[name="time_one"]').val())
									$('input[name="time2"]').val($('input[name="time_two"]').val())
									$('input[name="day"]').val(days);

									$('input[name="repeat"]').val($('select[name="repeat"]').val())
									$('input[name="remind"]').val($('select[name="remind"]').val())
								}
								// callback(eventsList);
								$('.addRec').modal('hide')
							}
				...
				$('#calendar').fullCalendar({
					...
					selectConstraint: {
						...
						end: $.fullCalendar.moment().startOf('year').add(10, 'year')
					},
					select: function(start, end, allDay) {
						$('.addRec').modal('show')
						$('.date').datepicker(
							"setDate", $.fullCalendar.formatDate(start, "MM-DD-YYYY"),
						);
